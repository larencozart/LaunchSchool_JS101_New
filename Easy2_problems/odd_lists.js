/*
Write a function that returns an Array that contains
every other element of an Array that is passed in as
an argument. The values in the returned list should be
those values that are in the 1st, 3rd, 5th, and so on
elements of the argument Array.
*/

function oddities1 (arr) {
  let oddArr = [];
  for (let idx = 0; idx < arr.length; idx += 1) {
    if (idx % 2 === 0) {
      oddArr.push(arr[idx]);
    }
  }
  return oddArr;
}

let oddities2 = arr => arr.filter((_element, idx) => idx % 2 === 0);


console.log(oddities1([2, 3, 4, 5, 6])); // logs [2, 4, 6]
console.log(oddities1([1, 2, 3, 4, 5, 6])); // logs [1, 3, 5]
console.log(oddities1(["abc", "def"])); // logs ['abc']
console.log(oddities1([123])); // logs [123]
console.log(oddities1([])); // logs []

console.log(oddities2([2, 3, 4, 5, 6])); // logs [2, 4, 6]
console.log(oddities2([1, 2, 3, 4, 5, 6])); // logs [1, 3, 5]
console.log(oddities2(["abc", "def"])); // logs ['abc']
console.log(oddities2([123])); // logs [123]
console.log(oddities2([])); // logs []